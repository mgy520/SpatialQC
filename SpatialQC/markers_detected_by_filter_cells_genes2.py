...
@marker_decorator
def detect_markers_by_filter_cells2(adata, markers, min_genes_list=None, min_cells_list=None):
    ...
    for frac in min_cells_list:
        fig.add_trace(go.Scatter(x=min_genes_list, y=gene_overlap[frac], mode='lines', name=f'min_cells={frac}'))

    fig.update_layout(
        title=dict(text='Detected markers post filter', x=0.5, y=0.9),
        legend=dict(yanchor="top", xanchor="right", x=0.99, y=0.99, bgcolor='rgba(0,0,0,0)'),
...
def create_plot(adata, markers, species=None, tissue_class=None, tissue_type=None,
    ...
    data_fig = detect_markers_by_filter_cells2(adata, markers, species, tissue_class, tissue_type, cancer_type,
